...
public class Packet implements Cloneable {
    ...
    public <T> Packet write(T value) {
        ...
        switch (value.getClass().getSimpleName().toLowerCase()) {
            ...
            case "double":
                ...
                break;
            default:
                throw new UnsupportedTypeException(value.getClass().getTypeName());
        }
    ...
    public <T> T read(Class<T> mClass) {
        ...
        switch (mClass.getSimpleName().toLowerCase()) {
            ...
            case "double":
                ...
                break;
            default:
                throw new UnsupportedTypeException(mClass.getTypeName());
        }
    ...
    public <T> T toObject(Class<T> mClass) {
        try {
            ...
            Stream<Field> fields = Stream.of(target.getClass().getDeclaredFields());

            fields.filter(f -> (f.isAnnotationPresent(Obtain.class) && f.getAnnotation(Obtain.class).value()))
    ...
    public Packet fromObject(Object target) {
        ...
        fields.filter(f -> (f.isAnnotationPresent(Obtain.class) && f.getAnnotation(Obtain.class).value()))
                .sorted(Comparator.comparing(Field::getName)).forEach(f -> {
            f.setAccessible(true);
            try {
                write(f.get(target));
            } catch (IllegalAccessException e) {
